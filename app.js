(
  function() {
      ...
      app.post('/user/add', function(req, res) {
          ...
          const db = new sqlite3.Database('database.db');
          db.run('INSERT INTO users (firstName,lastName,pubishName) VALUES (?)', [firstName,lastName,pubishName], (err) => {
              if (err) {
                  throw err;
              }else{
      ...
      app.put('/user/update/:id', function(req, res) {
          ...
          const { user_name } = req.body
          const db = new sqlite3.Database('database.db');
          db.run('UPDATE users SET firstName=? WHERE user_id=?', [firstName, id], (err) => {
              if (err) {
                  throw err;
              }else{
